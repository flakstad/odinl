...
`with-*`-style cleanup, and repetitive check/error propagation. `when-let` is
the first explicit multi-return convenience macro: it expands
`[value condition expr]` to a destructuring `let` plus `when condition`.
